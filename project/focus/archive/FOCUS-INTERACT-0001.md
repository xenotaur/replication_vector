...
# FOCUS-INTERACT-0001: Controllable Parent-Probe Tuning Sandbox
...
- `FOCUS-REPLAY-0001` was complete: `WI-REPLAY-0001` resolved deterministic parent-probe motion replay through Velumin and `EV-0008` records the replay artifact evidence.
- The Rust simulation model already exposed normalized thrust/turn input and tunable motion configuration.
- The browser harness already rendered project-owned Velumin frames, but did not yet support live parent-probe control.
- A narrow tuning sandbox let maintainers evaluate heavy motion feel before committing to mining, collision, shield, enemy, or child-probe gameplay.
...
## Completion Evidence
- `WI-INTERACT-0001` resolved the controllable parent-probe tuning sandbox.
- `EV-0009` records the browser/WebGPU smoke evidence for keyboard-controlled parent-probe motion and live tuning sliders.
- `scripts/README.md` documents the opt-in sandbox route and `scripts/render-sandbox-smoke` command.
